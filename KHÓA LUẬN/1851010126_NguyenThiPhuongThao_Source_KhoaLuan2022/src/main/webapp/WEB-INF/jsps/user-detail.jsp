<%-- 
    ...
    Created on : 23 Sept 2021, 21:21:35
    Author     : ntpth
--%>
...
<div class="row" style="background-color: #F8F8FF;">
    ...
    <div class="col-md-8">
        <h1 class="text-center" style="color:#00BFFF;"><spring:message code="label.staff.update" /></h1>
        ...
        <c:if test="${userId != 0}">
            <c:url value="/user-detail/${userId}" var="action" />
        </c:if>
        <c:if test="${userId == null}">
            <c:url value="/user-detail/${userdetail.id}" var="action" />
        </c:if>

        <form:form method="post" action="${action}" modelAttribute="userdetail" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <img style="cursor: pointer; width: 30%; height: 10%" class="img-fluid" src="<c:url value="${userdetail.image}"/>" alt="${userdetail.id}" />
            </div>
            <div class="form-group">
